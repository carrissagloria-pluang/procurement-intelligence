# -*- coding: utf-8 -*-
"""git - report generation 3.0.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1EhZxqUn75-JRpD9xQKOajF4jk3zb20Vn
...
def get_token(SCOPES):
  ...
  # Get the URL to click
  auth_url, _ = flow.authorization_url(prompt='consent')
  print(f'Please go to this URL and authorize: {auth_url}')

  # enter box for the authentication code
  code = input('Enter the authorization code: ')

  # Exchange the code for the final token
...
def authenticate_github(SCOPES):
  creds = None

  # Try to load from Environment Variables (GitHub Actions)
  ...
  # Fallback to local files (Development/Testing)
...
def process_data(creds, SPREADSHEET_ID, RANGE_NAME):
...
def generate_report(chat, tender_data):
...
def upload_markdown_as_doc(creds, title, markdown_content):
    ...
    # Convert your Markdown string into HTML
    html_content = markdown.markdown(markdown_content)
    folder_id = os.environ.get('DRIVE_FOLDER_ID')

    # Metadata stays the same (target is a Google Doc)
    file_metadata = {
        'name': title,
    ...
    # Set source mimetype to 'text/html'
...
def main():
  ...
  # generate the intial token to local files
  # get_token(SCOPES)

  # authenticate the connections using the token
  # Build your services
  creds = authenticate_github(SCOPES)
  ...
  # loop through each tender item that needs a report
  for i in gen_report:
    # get the data
    tender_data = tender_df.iloc[i]

    # prompt gemini for the report
